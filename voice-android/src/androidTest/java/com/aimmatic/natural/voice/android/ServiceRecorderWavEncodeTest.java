...
public class ServiceRecorderWavEncodeTest {
    ...
    @Test
    public void testWavRecordFullDuration() throws Exception {
        testWavRecordService(serviceRule, "Max 59 ", 59,
                59 * 1000, 59 * 1000, false, false);
        testWavRecordService(serviceRule, "Max 29 ", 29,
                29 * 1000, 29 * 1000, false, false);
    }

    @Test
    public void testWavRecordRandomStop() throws Exception {
        testWavRecordService(serviceRule, "Max 29 speech timeout ", -1,
                29 * 1000, 10 * 1000, true, false);
        testWavRecordService(serviceRule, "Max 29 random stop ", -1,
     ...
     * @throws Exception
     */
    void testWavRecordService(ServiceTestRule serviceRule, String tcase, final int expectedDuration, final int maxDuration, final int minDuration,
        ...
        bout.close();
        WavVerifier wavVerifier = WavVerifier.getWavVerifier(data);
        // testing
        ...
        Assert.assertTrue(tcase + "Wrong wav header", wavVerifier.isValid());
        Assert.assertTrue(tcase + "Wrong sample rate expect " + mockAudioRecord.getSampleRate() + " got " + wavVerifier.getSampleRate(), wavVerifier.getSampleRate() == mockAudioRecord.getSampleRate());
        int verifyDuration = expectedDuration;
        if (expectedDuration == -1) {
            verifyDuration = mockAudioRecord.getTotalDuration() / 1000;
        }
        Assert.assertTrue(tcase + "Wrong duration expect " + verifyDuration + " got " + wavVerifier.getDuration(), wavVerifier.getDuration() == verifyDuration);
    }
